...
import { describeAdapterExecutionTarget, ensureAdapterExecutionTargetCommandResolvable, ensureAdapterExecutionTargetDirectory, resolveAdapterExecutionTargetCwd, runAdapterExecutionTargetProcess, } from "@paperclipai/adapter-utils/execution-target";
...
const GROK_AUTH_REQUIRED_RE = /(?:not\s+logged\s+in|login\s+required|run\s+`?grok\s+login`?|authentication\s+required|unauthorized|invalid\s+credentials)/i;
...
export async function testEnvironment(ctx) {
    ...
    const canRunProbe = checks.every((check) => check.code !== "grok_cwd_invalid" && check.code !== "grok_command_unresolvable");
