...
async function checkApiKeys(config, detectedConfig) {
    ...
    const has = (key) => !!(resolvedEnv[key] ?? process.env[key] ?? hermesEnvKeys[key]);
    ...
    const supportedProviders = VALID_PROVIDERS;
    const modelMatchesRequested = !!detectedConfig?.model &&
        (!requestedModel || detectedConfig.model.toLowerCase() === requestedModel.toLowerCase());
    const matchingHermesConfigApiKey = !!detectedConfig?.hasApiKey &&
 ...
 */
async function checkProviderConsistency(config, detectedConfig) {
    const model = asString(config.model);
    if (!model)
        return null;
