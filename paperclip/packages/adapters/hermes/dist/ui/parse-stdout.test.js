...
describe("parseHermesStdoutLine — ANSI stripping", () => {
    it("strips 24-bit foreground + background color CSI sequences", () => {
        const result = parseHermesStdoutLine("\x1b[38;2;255;255;255;48;2;19;87;20m+r = curl(\"POST\", \"/api/issues/d7b08cc5/comments\",\x1b[0m", TS);
    ...
    it("strips light text CSI sequence", () => {
        const result = parseHermesStdoutLine("\x1b[38;2;255;248;220mAll done. Now let me verify.\x1b[0m", TS);
        expect(result).toHaveLength(1);
    ...
    it("strips multiple CSI sequences on a single line", () => {
        const result = parseHermesStdoutLine("\x1b[38;2;255;255;255;48;2;19;87;20m+ \"priority\": \"highest\",\x1b[0m \x1b[38;2;255;255;255;48;2;19;87;20m+r = curl(\"PATCH\", ...\x1b[0m", TS);
        expect(result.length).toBeGreaterThan(0);
        for (const entry of result) {
            for (const v of Object.values(entry)) {
                if (typeof v === "string") {
