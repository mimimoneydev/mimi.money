...
describe("execute", () => {
    ...
    it("routes a bare Hermes dashboard URL on port 9119 through the API prefix", async () => {
        ...
        expect(ctx.onLog.mock.calls.map(([, line]) => String(line)).join("\n"))
    ...
    it("redacts echoed auth material from stream logs and summaries", async () => {
        ...
        vi.stubGlobal("fetch", fetchMock);
        const result = await execute(ctx);
        const logText = ctx.onLog.mock.calls.map(([, line]) => String(line)).join("\n");
        expect(result.exitCode).toBe(0);
...
describe("testEnvironment", () => {
    ...
    it("tests a bare Hermes dashboard URL on port 9119 through the API prefix", async () => {
        ...
        expect(fetchMock).toHaveBeenCalledWith("http://127.0.0.1:9119/api/health", expect.objectContaining({ method: "GET" }));
    });
