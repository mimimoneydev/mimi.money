...
import { DEFAULT_PAPERCLIP_AGENT_PROMPT_TEMPLATE, asBoolean, asString, buildPaperclipEnv, joinPromptSections, parseObject, readPaperclipIssueWorkModeFromContext, renderPaperclipWakePrompt, isPaperclipRecoveryWakePayload, renderTemplate, stringifyPaperclipWakePayload, } from "@paperclipai/adapter-utils/server-utils";
...
export async function execute(ctx) {
    ...
    const renderedBootstrapPrompt = !canReuseSession && bootstrapPromptTemplate.trim().length > 0
        ...
        : "";
    const renderedPrompt = (canReuseSession && wakePrompt.length > 0) || isPaperclipRecoveryWakePayload(context.paperclipWake)
        ? ""
