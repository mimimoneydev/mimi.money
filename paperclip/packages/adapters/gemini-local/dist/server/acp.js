...
export function resolveGeminiExecutionEngine(config) {
    return normalizeEngine(config.engine);
}
export async function resolveGeminiExecutionEngineForRun(input) {
...
export function formatGeminiAcpFallbackMessage(reason) {
...
export function buildGeminiAcpConfig(config) {
...
function withGeminiAcpDefaults(options) {
    return {
        ...options,
...
export function createGeminiAcpExecutor(options = {}) {
    let executor = null;
    return async (ctx) => {
...
export function nodeVersionMeetsGeminiAcpMinimum(version = process.version) {
...
async function findCommandOnPath(binName, pathValue = process.env.PATH ?? "") {
...
async function commandIsResolvable(command, pathValue = process.env.PATH ?? "", input) {
    ...
    if (target?.kind === "remote") {
        try {
            await ensureAdapterExecutionTargetCommandResolvable(token, target, resolveAdapterExecutionTargetCwd(target, asString(input?.config.cwd, ""), process.cwd()), process.env);
            return true;
        }
        catch {
            return false;
    ...
    if (path.isAbsolute(token) || hasPathSeparator(token))
        return pathExists(token);
    return (await findCommandOnPath(token, pathValue)) !== null;
}
function resolveGeminiAcpCommand(config) {
...
async function defaultGeminiAcpFallbackReason(input) {
    const target = readAdapterExecutionTarget({
        executionTarget: input.executionTarget,
        legacyRemoteExecution: input.executionTransport?.remoteExecution,
    });
...
export async function testGeminiAcpEnvironment(ctx) {
    const checks = [];
