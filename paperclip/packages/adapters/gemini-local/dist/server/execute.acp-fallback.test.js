...
const { ensureAdapterExecutionTargetCommandResolvable, ensureAdapterExecutionTargetRuntimeCommandInstalled, executeGeminiAcp, readPaperclipRuntimeSkillEntries, resolveAdapterExecutionTargetCommandForLogs, runAdapterExecutionTargetProcess, } = vi.hoisted(() => ({
...
vi.mock("./acp.js", () => ({
    ...
    formatGeminiAcpFallbackMessage: (reason) => `[paperclip] Gemini ACP default unavailable; falling back to Gemini CLI. ${reason} Set engine=acp to require ACP or engine=cli to silence this fallback.\n`,
    resolveGeminiExecutionEngineForRun: async (ctx) => ctx.config.engine === "acp"
...
describe("gemini_local ACP startup fallback", () => {
    ...
    it("falls back to Gemini CLI when auto-selected ACP fails before execution starts", async () => {
        ...
        expect(ctx.onLog).toHaveBeenCalledWith("stderr", expect.stringContaining("Gemini ACP startup failed"));
