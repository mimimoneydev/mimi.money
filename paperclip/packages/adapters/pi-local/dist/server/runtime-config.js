...
function parseProviderConfig(raw, resolveEnv) {
...
export async function preparePiRuntimeConfig(input) {
    ...
    const { providers, warning } = parseProviderConfig(input.env.PAPERCLIP_PI_PROVIDERS ?? process.env.PAPERCLIP_PI_PROVIDERS, resolveEnv);
    if (!providers) {
        return {
            env: input.env,
    ...
    try {
        await fs.writeFile(path.join(agentConfigDir, "models.json"), `${JSON.stringify({ providers }, null, 2)}\n`, "utf8");
    }
    catch (err) {
