...
const effortFlagSupportCache = new Map();
export function claudeCommandLooksLike(command, expected = "claude") {
    const base = path.basename(command).toLowerCase();
    return base === expected || base === `${expected}.cmd` || base === `${expected}.exe`;
}
function cacheKeyForTarget(command, target) {
    if (!target)
...
async function probeClaudeCommandSupportsEffortFlag(input) {
    const help = await runAdapterExecutionTargetProcess(input.runId, input.target, input.command, ["--help"], {
        cwd: input.cwd,
        env: input.env,
    ...
    if (output.includes("--effort"))
        return true;
    if ((help.exitCode ?? 0) === 0)
        return false;
    return null;
}
export async function claudeCommandSupportsEffortFlag(input) {
    if (!claudeCommandLooksLike(input.command, "claude"))
        return null;
