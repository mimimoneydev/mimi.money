...
import { asString, asBoolean, asNumber, asStringArray, parseJson, parseObject, ensurePathInEnv, } from "@paperclipai/adapter-utils/server-utils";
import { ensureAdapterExecutionTargetCommandResolvable, ensureAdapterExecutionTargetDirectory, maybeRunSandboxInstallCommand, prepareAdapterExecutionTargetRuntime, runAdapterExecutionTargetProcess, describeAdapterExecutionTarget, resolveAdapterExecutionTargetCwd, adapterExecutionTargetUsesManagedHome, } from "@paperclipai/adapter-utils/execution-target";
import { describeClaudeFailure, detectClaudeLoginRequired, isClaudeTransientUpstreamError, parseClaudeStreamJson, } from "./parse.js";
...
function lastNonInitStdoutLine(text) {
    const lines = text
        .split(/\r?\n/)
        .map((line) => line.trim())
        .filter(Boolean);
...
function truncateDetail(value, max = 240) {
...
export async function testEnvironment(ctx) {
    ...
    if (targetIsRemote && adapterExecutionTargetUsesManagedHome(target) && !hasExplicitClaudeConfigDir) {
        ...
        try {
            ...
            const runtimeRootDir = preparedRuntime.runtimeRootDir ?? path.posix.join(managedRemoteCwd, ".paperclip-runtime", "claude");
            const remoteClaudeConfigSeedDir = preparedRuntime.assetDirs["config-seed"] ?? path.posix.join(runtimeRootDir, "config-seed");
    ...
    const canRunProbe = checks.every((check) => check.code !== "claude_cwd_invalid" &&
    ...
    if (canRunProbe) {
        if (!claudeCommandLooksLike(command, "claude")) {
            ...
            if (probe.timedOut) {
                ...
                const failureDetail = (parsed ? describeClaudeFailure(parsed) : null) ||
