...
import { asString, asNumber, asBoolean, asStringArray, parseObject, parseJson, applyPaperclipWorkspaceEnv, buildPaperclipEnv, readPaperclipRuntimeSkillEntries, readPaperclipIssueWorkModeFromContext, joinPromptSections, buildInvocationEnvForLogs, ensureAbsoluteDirectory, ensurePathInEnv, isForbiddenConfigEnvKey, isPaperclipRuntimeEnvKey, refreshPaperclipWorkspaceEnvForExecution, renderTemplate, renderPaperclipWakePrompt, isPaperclipRecoveryWakePayload, rewriteWorkspaceCwdEnvVarsForExecution, shapePaperclipWorkspaceEnvForExecution, stringifyPaperclipWakePayload, DEFAULT_PAPERCLIP_AGENT_PROMPT_TEMPLATE, } from "@paperclipai/adapter-utils/server-utils";
import { parseLocalProcessFilesystemScope, parseLocalProcessSandboxExtraPaths, parseLocalProcessNetworkAllowlist, parseLocalProcessNetworkScope, } from "@paperclipai/adapter-utils/local-process-sandbox";
import { claudeModelUsageTotals, parseClaudeStreamJson, describeClaudeFailure, detectClaudeLoginRequired, extractClaudeRetryNotBefore, isClaudeMaxTurnsResult, isClaudeProviderQuotaError, isClaudeRefusalResult, isClaudeTransientUpstreamError, isClaudeUnknownSessionError, isClaudePoisonedPreviousMessageIdError, isClaudeImageProcessingError, isClaudeModelNotFoundError, } from "./parse.js";
import { materializeRemoteClaudeConfig, prepareClaudeConfigSeed, resolveManagedClaudeRuntimeStateDir, resolveSharedClaudeConfigDir, writePaperclipClaudeMcpConfig, } from "./claude-config.js";
...
import { createClaudeAcpExecutor, formatClaudeAcpFallbackMessage, resolveClaudeExecutionEngineForRun, } from "./acp.js";
...
async function buildClaudeRuntimeConfig(input) {
    ...
    for (const [key, value] of Object.entries(shapedEnvConfig)) {
        ...
        if (isForbiddenConfigEnvKey(key))
            continue;
        if (isPaperclipRuntimeEnvKey(key) && key in env)
            continue;
        env[key] = value;
    }
    if (authToken) {
...
export async function execute(ctx) {
    ...
    if (engineSelection.engine === "acp") {
        try {
            ...
            await ctx.onLog("stderr", formatClaudeAcpFallbackMessage(`Claude ACP startup failed: ${reason}`));
        }
    }
    ...
    const runtimeMcpIdentity = JSON.stringify(runtimeMcpServers.map(({ name, url, connectionId }) => ({ name, url, connectionId })));
    const claudeRuntimeStateDir = resolveManagedClaudeRuntimeStateDir(process.env, agent.companyId, agent.id);
    ...
    const localProcessSandbox = (filesystemScope || networkScope) && !executionTargetIsRemote
        ? {
    ...
    if (localProcessSandbox) {
        ...
        const scopes = [filesystemScope ? "workspace filesystem" : null, networkScope ? `${networkScope} network` : null]
            .filter(Boolean)
            .join(" and ");
        await onLog("stdout", `[paperclip] Confining Claude with ${scopes} scope.\n`);
    }
    ...
    const effectiveMcpConfigPath = executionTargetIsRemote
        ? path.posix.join(preparedExecutionTargetRuntime?.assetDirs["mcp-config"] ??
            path.posix.join(effectiveExecutionCwd, ".paperclip-runtime", "claude", "mcp-config"), path.basename(localMcpConfigPath))
        : localMcpConfigPath;
    ...
    const hasMatchingMcpServers = runtimeMcpServerIdentity.length === 0
    ...
    if (runtimeSessionId && !hasMatchingMcpServers) {
        await onLog("stdout", `[paperclip] Claude session "${runtimeSessionId}" was saved with a different runtime MCP server set and will not be resumed.\n`);
    }
    ...
    const runAttempt = async (resumeSessionId) => {
        ...
        if (runtimeMcpServers.length > 0) {
            commandNotes.push(`Using ${runtimeMcpServers.length} Paperclip-managed MCP server(s) from strict config ${effectiveMcpConfigPath}.`);
        }
        if (onMeta) {
            await onMeta({
    ...
    const toAdapterResult = (attempt, opts) => {
        ...
        if (!parsed) {
            ...
            const providerQuota = !loginMeta.requiresLogin &&
                (proc.exitCode ?? 0) !== 0 &&
