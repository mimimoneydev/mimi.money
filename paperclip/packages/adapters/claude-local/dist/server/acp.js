...
export function resolveClaudeExecutionEngine(config) {
    return normalizeEngine(config.engine);
}
export async function resolveClaudeExecutionEngineForRun(input) {
...
export function formatClaudeAcpFallbackMessage(reason) {
...
export function buildClaudeAcpConfig(config) {
 ...
 */
export function resolveClaudeAcpBillingIdentity(ctx) {
...
function withClaudeAcpDefaults(options) {
    return {
...
export function nodeVersionMeetsClaudeAcpMinimum(version = process.version) {
...
async function resolveClaudeAcpCommand(config) {
    const configured = firstNonEmptyString(config.agentCommand, config.acpAgentCommand);
    if (configured)
        return configured;
    return ((await findAncestorBin(packageRootDir, "claude-agent-acp")) ??
...
async function resolveClaudeAcpCommandForTarget(config, target) {
    const configured = firstNonEmptyString(config.agentCommand, config.acpAgentCommand);
    if (configured)
        return configured;
    if (target?.kind === "remote")
...
async function defaultClaudeAcpFallbackReason(input) {
    const target = readAdapterExecutionTarget({
        executionTarget: input.executionTarget,
        legacyRemoteExecution: input.executionTransport?.remoteExecution,
    });
...
export async function testClaudeAcpEnvironment(ctx) {
    const checks = [];
    ...
    const hasBedrock = envConfig.CLAUDE_CODE_USE_BEDROCK === "1" ||
