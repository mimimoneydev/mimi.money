...
import { buildClaudeAcpConfig, createClaudeAcpExecutor, nodeVersionMeetsClaudeAcpMinimum, resolveClaudeAcpBillingIdentity, resolveClaudeExecutionEngine, resolveClaudeExecutionEngineForRun, testClaudeAcpEnvironment, } from "./acp.js";
const tempRoots = [];
...
class FakeRuntime {
    ...
    ], terminal = { status: "completed", stopReason: "end_turn" }) {
...
async function createRuntimeSkill(root) {
...
function buildContext(root, overrides = {}) {
    return {
        runId: "run-1",
        agent: {
            id: "agent-1",
...
describe("claude_local ACP lane", () => {
    ...
    it("executes through ACPX with Claude model env, settings.local.json, and ephemeral skills", async () => {
        ...
        const skillRoot = (result.sessionParams?.skills).skillRoot;
...
describe("resolveClaudeAcpBillingIdentity", () => {
    ...
    afterEach(() => {
        ...
        if (originalBedrockBase === undefined)
            ...
            process.env.ANTHROPIC_BEDROCK_BASE_URL = originalBedrockBase;
    });
    it("classifies an adapter-config API key as api billing", () => {
        expect(resolveClaudeAcpBillingIdentity({ config: { env: { ANTHROPIC_API_KEY: "sk-ant-test" } } })).toEqual({ provider: "anthropic", biller: "anthropic", billingType: "api" });
    });
    it("classifies Bedrock auth as metered_api billed to aws_bedrock", () => {
        expect(resolveClaudeAcpBillingIdentity({ config: { env: { CLAUDE_CODE_USE_BEDROCK: "1" } } })).toEqual({ provider: "anthropic", biller: "aws_bedrock", billingType: "metered_api" });
    });
    ...
    it("ignores host env for remote execution targets", () => {
        ...
        expect(resolveClaudeAcpBillingIdentity({
            config: {},
            executionTarget: { kind: "remote", transport: "sandbox", remoteCwd: "/work" },
        }).billingType).toBe("subscription");
    });
});
