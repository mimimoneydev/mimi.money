...
export function buildClaudeLocalConfig(v) {
    const ac = {};
    if (v.claudeEngine === "cli")
        ac.engine = "cli";
    if (v.claudeEngine === "acp") {
        ac.engine = "acp";
        if (v.claudeAcpAgentCommand)
            ac.agentCommand = v.claudeAcpAgentCommand;
        if (v.claudeAcpMode)
            ac.mode = v.claudeAcpMode;
        if (v.claudeAcpNonInteractivePermissions) {
            ac.nonInteractivePermissions = v.claudeAcpNonInteractivePermissions;
        }
        if (v.claudeAcpStateDir)
            ac.stateDir = v.claudeAcpStateDir;
