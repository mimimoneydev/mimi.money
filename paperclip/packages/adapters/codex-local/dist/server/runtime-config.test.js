...
async function makeCodexHome(configToml) {
...
async function readConfigToml(home) {
...
describe("prepareCodexRuntimeConfig", () => {
    ...
    it("preserves existing config.toml content, keeps model_provider in the root region, and restores on cleanup", async () => {
        ...
        expect(content.indexOf('model_provider = "bifrost"')).toBeLessThan(content.indexOf("[profiles.dev]"));
        expect(content.indexOf("[model_providers.bifrost]")).toBeGreaterThan(content.indexOf("[profiles.dev]"));
        await prepared.cleanup();
    ...
    it("surfaces skipped non-object provider entries while merging the usable ones", async () => {
        ...
        expect(prepared.notes.some((n) => n.includes("skipped provider(s) with empty names or non-object values: bad"))).toBe(true);
    ...
    it("rejects the block when model_provider names a filtered or missing provider", async () => {
        ...
        expect(prepared.notes).toContain('PAPERCLIP_CODEX_PROVIDERS: model_provider "bad" does not match any usable provider entry; custom providers ignored.');
        const content = await readConfigToml(home);
