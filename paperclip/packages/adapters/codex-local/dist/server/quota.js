...
class CodexQuotaAuthError extends Error {
    errorFamily;
    constructor(message, errorFamily) {
        super(message);
...
async function readResponseTextPrefix(resp, maxBytes = MAX_QUOTA_ERROR_BODY_BYTES) {
    if (maxBytes <= 0 || !resp.body)
        return "";
...
export async function fetchCodexQuota(token, accountId) {
    ...
    if (!resp.ok) {
        ...
        if (authFailure)
            throw new CodexQuotaAuthError(message, authFailure);
...
export function readCodexQuotaErrorFamily(error) {
    if (error instanceof CodexQuotaAuthError)
        return error.errorFamily;
    const message = error instanceof Error ? error.message : String(error);
    return classifyCodexAuthRefreshFailure({ errorMessage: message });
}
export async function getQuotaWindows() {
    const errors = [];
    let rpcErrorFamily = null;
    try {
