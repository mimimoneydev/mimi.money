...
describe("codex remote execution", () => {
    ...
    it("prepares the workspace, syncs CODEX_HOME, and restores workspace changes for remote SSH execution", async () => {
        ...
        const homeSyncArgs = syncDirectoryToSsh.mock.calls[0]?.[0];
    ...
    it("stages only the allowlist into the home asset: keeps config.toml/skills/auth, drops session+sqlite state, no exclude", async () => {
        ...
        await writeFile(path.join(codexHomeDir, "config.toml"), 'model_provider = "bifrost"\n\n[model_providers.bifrost]\nname = "bifrost"\n', "utf8");
        ...
        syncDirectoryToSsh.mockImplementationOnce(async (args) => {
        ...
        expect(snap.entries).toEqual(["auth.json", "config.json", "config.toml", "instructions.md", "skills"]
