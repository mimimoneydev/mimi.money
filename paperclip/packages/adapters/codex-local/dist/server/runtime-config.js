...
// left intact.
function expandEnvPlaceholders(value, resolve) {
    ...
    if (Array.isArray(value)) {
        return value.map((entry) => expandEnvPlaceholders(entry, resolve));
    }
...
function parseCodexProvidersConfig(raw, resolveEnv, notes) {
    ...
    if (!isPlainObject(rawProviders)) {
        notes.push('PAPERCLIP_CODEX_PROVIDERS has no "providers" object; custom providers ignored.');
        return null;
    }
    ...
    if (Object.keys(providers).length === 0) {
        notes.push(`PAPERCLIP_CODEX_PROVIDERS "providers" contains no usable entries${skipped.length > 0
            ? ` (skipped provider(s) with empty names or non-object values: ${skipped.join(", ")})`
            : ""}; custom providers ignored.`);
        return null;
    }
    if (skipped.length > 0) {
        notes.push(`PAPERCLIP_CODEX_PROVIDERS: skipped provider(s) with empty names or non-object values: ${skipped.join(", ")}.`);
    }
    const modelProvider = typeof parsed.model_provider === "string" && parsed.model_provider.trim().length > 0
    ...
    if (modelProvider !== null && !(modelProvider in providers)) {
        notes.push(`PAPERCLIP_CODEX_PROVIDERS: model_provider "${modelProvider}" does not match any usable provider entry; custom providers ignored.`);
        return null;
    }
...
function emitProviderTable(name, fields) {
...
export function stripManagedCodexProviderBlocks(content) {
...
function parseTableHeaderPath(line) {
...
function stripConflictingDefinitions(content, providerNames, removeRootModelProvider) {
...
function buildMergedConfigToml(base, parsed) {
    const sections = [];
...
function configTomlBackupPath(configTomlPath) {
...
export async function prepareCodexRuntimeConfig(input) {
    const resolveEnv = (name) => input.env[name] ?? process.env[name];
    const notes = [];
    const parsed = parseCodexProvidersConfig(input.env.PAPERCLIP_CODEX_PROVIDERS ?? process.env.PAPERCLIP_CODEX_PROVIDERS, resolveEnv, notes);
    if (!parsed) {
    ...
    const base = stripConflictingDefinitions(stripManagedCodexProviderBlocks(original ?? ""), providerNames, parsed.modelProvider !== null);
    ...
    return {
        notes: [
            ...notes,
            `Merged ${providerNames.length} custom Codex model provider(s) from PAPERCLIP_CODEX_PROVIDERS into "${configTomlPath}": ${providerNames.join(", ")}${parsed.modelProvider ? `; selected model_provider "${parsed.modelProvider}"` : ""}.`,
        ],
        cleanup: async () => {
            if (original === null) {
