...
import { buildCodexAcpConfig, createCodexAcpExecutor, nodeVersionMeetsCodexAcpMinimum, resolveCodexAcpBillingIdentity, resolveCodexExecutionEngine, resolveCodexExecutionEngineForRun, testCodexAcpEnvironment, } from "./acp.js";
const tempRoots = [];
...
describe("codex_local ACP lane", () => {
    ...
    it("executes through ACPX with Codex session config and ephemeral skills", async () => {
        ...
        const skillsHome = (result.sessionParams?.skills).skillsHome;
...
describe("resolveCodexAcpBillingIdentity", () => {
    ...
    it("classifies an adapter-config API key as api billing to openai", () => {
        delete process.env.OPENROUTER_API_KEY;
        expect(resolveCodexAcpBillingIdentity({ config: { env: { OPENAI_API_KEY: "sk-test" } } })).toEqual({ provider: "openai", biller: "openai", billingType: "api" });
    });
    ...
    it("bills OpenRouter-backed runs to openrouter", () => {
        expect(resolveCodexAcpBillingIdentity({
