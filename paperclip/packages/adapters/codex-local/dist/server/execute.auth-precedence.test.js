...
import { CODEX_SANDBOX_AUTH_EXISTS_COMMAND, CODEX_SANDBOX_AUTH_PRECEDENCE_WARNING, CODEX_SANDBOX_AUTH_PRECEDENCE_WARNING_LOG_LINE, } from "./auth-precedence.js";
const { ensureAdapterExecutionTargetCommandResolvable, ensureAdapterExecutionTargetRuntimeCommandInstalled, prepareAdapterExecutionTargetRuntime, resolveAdapterExecutionTargetCommandForLogs, runAdapterExecutionTargetProcess, runAdapterExecutionTargetShellCommand, startAdapterExecutionTargetPaperclipBridge, } = vi.hoisted(() => ({
...
describe("codex sandbox auth precedence warning", () => {
    ...
    it("logs and emits a run event when sandbox login is shadowed by host auth", async () => {
        ...
        await fs.writeFile(path.join(hostCodexHome, "auth.json"), JSON.stringify({ OPENAI_API_KEY: "fake-host-auth" }), "utf8");
        const logs = [];
        const events = [];
        await execute({
        ...
        const runtimeCall = prepareAdapterExecutionTargetRuntime.mock.calls[0]?.[0];
        ...
        expect(runAdapterExecutionTargetShellCommand).toHaveBeenCalledWith("run-auth-precedence", expect.objectContaining({ kind: "remote", transport: "sandbox", remoteCwd: "/sandbox/workspace" }), CODEX_SANDBOX_AUTH_EXISTS_COMMAND, expect.objectContaining({ env: {}, timeoutSec: 5 }));
...
//# sourceMappingURL=execute.auth-precedence.test.js.map