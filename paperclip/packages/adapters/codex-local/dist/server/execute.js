...
import { parseCodexJsonl, classifyCodexAuthRefreshFailure, extractCodexRetryNotBefore, isCodexProviderQuotaError, isCodexTransientUpstreamError, isCodexUnknownSessionError, } from "./parse.js";
import { codexHomeHasUsableAuth, evaluateCodexCredentialReadiness, isManagedCodexHomePath, pathExists, prepareManagedCodexHome, resolveManagedCodexHomeDir, resolveSharedCodexHomeDir, seedManagedCodexHome, stageCodexHomeForSync, mergeManagedCodexMcpGateways, writeManagedCodexMcpConfig, } from "./codex-home.js";
import { CODEX_SANDBOX_AUTH_EXISTS_COMMAND, CODEX_SANDBOX_AUTH_PRECEDENCE_WARNING, CODEX_SANDBOX_AUTH_PRECEDENCE_WARNING_LOG_LINE, resolveCodexAuthPrecedence, } from "./auth-precedence.js";
...
import { createCodexAcpExecutor, formatCodexAcpFallbackMessage, resolveCodexExecutionEngineForRun, } from "./acp.js";
...
function managedMcpGatewaysFromContext(context) {
    const managedMcp = parseObject(context.paperclipManagedMcp);
    if (managedMcp.managedMcpOnly !== true)
        return [];
    ...
    return gateways
        ...
        if (!name || !endpointPath || !bearerToken)
            return null;
        return { name, endpointPath, bearerToken };
    })
        .filter((gateway) => Boolean(gateway));
}
async function sandboxCodexAuthJsonExists(input) {
    ...
    try {
        const result = await runAdapterExecutionTargetShellCommand(input.runId, input.target, CODEX_SANDBOX_AUTH_EXISTS_COMMAND, {
            cwd: input.cwd,
            env: {},
            timeoutSec: 5,
        });
...
export async function execute(ctx) {
    ...
    if (engineSelection.engine === "acp") {
        try {
            ...
            await ctx.onLog("stderr", formatCodexAcpFallbackMessage(`Codex ACP startup failed: ${reason}`));
        }
    }
    ...
    try {
        ...
        const managedMcpGateways = mergeManagedCodexMcpGateways(runtimeMcpGateways, managedMcpGatewaysFromContext(context));
        ...
        if (managedMcpGateways.length > 0) {
            await onLog("stdout", `[paperclip] Wrote ${managedMcpGateways.length} managed MCP gateway(s) into Codex config "${managedMcp.configPath}".\n`);
        }
                    ...
                    assets: [
                        {
                            ...
                            restore: async ({ assetDir, readFile }) => void (await copyBackCodexAuth({
        ...
        if (localProcessSandbox) {
            const scopes = [filesystemScope ? "workspace filesystem" : null, networkScope ? `${networkScope} network` : null]
                .filter(Boolean)
                .join(" and ");
            await onLog("stdout", `[paperclip] Confining Codex with ${scopes} scope.\n`);
        }
        ...
        const toResult = (attempt, clearSessionOnMissingSession = false, isRetry = false) => {
            ...
            const transientRetryNotBefore = (attempt.proc.exitCode ?? 0) !== 0
                ...
                : null;
            const authRefreshFailure = (attempt.proc.exitCode ?? 0) !== 0
                ...
                : null;
            const providerQuota = (attempt.proc.exitCode ?? 0) !== 0 &&
                !authRefreshFailure &&
        ...
        if (stagedCodexHomeDir) {
            await fs.rm(stagedCodexHomeDir, { recursive: true, force: true }).catch(async (error) => {
                await onLog("stderr", `[paperclip] Failed to remove staged Codex home "${stagedCodexHomeDir}": ${error instanceof Error ? error.message : String(error)}\n`);
            });
        }
