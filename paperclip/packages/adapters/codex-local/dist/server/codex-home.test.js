...
import { CODEX_SYNC_ALLOWLIST, codexHomeHasUsableAuth, ensureSymlink, evaluateCodexCredentialReadiness, mergeManagedCodexMcpGateways, isManagedCodexHomePath, prepareManagedCodexHome, reconcileManagedCodexHome, seedManagedCodexHome, stageCodexHomeForSync, writeManagedCodexMcpConfig, } from "./codex-home.js";
describe("mergeManagedCodexMcpGateways", () => {
    it("keeps runtime gateways and appends non-overlapping context gateways", () => {
        expect(mergeManagedCodexMcpGateways([{ name: "runtime", endpointPath: "/runtime", bearerToken: "runtime-token" }], [
...
describe("evaluateCodexCredentialReadiness", () => {
    async function makeFixture() {
        ...
        const sharedCodexHome = path.join(root, "shared-codex-home");
        const paperclipHome = path.join(root, "paperclip-home");
        const companyRoot = path.join(paperclipHome, "instances", "default", "companies", "company-1");
    ...
    async function writeUsableAuth(home) {
...
describe("stageCodexHomeForSync", () => {
    ...
    async function buildFakeHome(root) {
    ...
    it("writes the staged config.toml (managed MCP bearer header) with mode 0600", async () => {
        ...
        try {
            ...
            await fs.writeFile(path.join(home, "config.toml"), "[mcp_servers.paperclip]\nheaders = { Authorization = \"Bearer secret-token\" }\n", { mode: 0o600 });
    ...
    it("fails closed and removes the temp dir on an unexpected I/O error", async () => {
        ...
        try {
            ...
            vi.spyOn(fs, "mkdtemp").mockImplementation(async (prefix, ...rest) => {
                const dir = await realMkdtemp(prefix, ...rest);
                createdDir = dir;
                return dir;
            });
            vi.spyOn(fs, "readFile").mockRejectedValue(Object.assign(new Error("boom"), { code: "EACCES" }));
    ...
    it("does not infinitely traverse an ancestor directory link (back -> .) inside a skill", async () => {
        ...
        try {
            ...
            expect(await fs.readFile(path.join(staged, "skills", "my-skill", "SKILL.md"), "utf8")).toBe("# skill\n");
        }
        finally {
            if (staged)
                await fs.rm(staged, { recursive: true, force: true });
