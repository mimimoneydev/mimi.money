...
const CODEX_AUTH_MERGE_EXTRACT_SCRIPT_BYTES = readFileSync(new URL(`./${CODEX_AUTH_MERGE_EXTRACT_SCRIPT_NAME}`, import.meta.url));
const CODEX_AUTH_MERGE_DECISION_SCRIPT_BYTES = readFileSync(new URL(`./${CODEX_AUTH_MERGE_DECISION_SCRIPT_NAME}`, import.meta.url));
/**
...
export function buildCodexAuthInboundProvision() {
    return {
        ...
        extractCommand: ({ assetTarPath, assetDir, runtimeRootDir }) => `sh ${shellQuote(path.posix.join(runtimeRootDir, CODEX_AUTH_MERGE_EXTRACT_SCRIPT_NAME))} ` +
            `${shellQuote(assetDir)} ${shellQuote(assetTarPath)}`,
    };
}
//# sourceMappingURL=codex-auth-merge-scripts.js.map