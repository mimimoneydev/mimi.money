...
describe("resolveCodexAuthPrecedence", () => {
    ...
    describe("constants", () => {
        it("warning message is stable and human-readable", () => {
            expect(CODEX_SANDBOX_AUTH_PRECEDENCE_WARNING).toBe("snapshot login present but configured or host credentials take precedence");
        });
        it("log line wraps the warning message", () => {
            expect(CODEX_SANDBOX_AUTH_PRECEDENCE_WARNING_LOG_LINE).toContain(CODEX_SANDBOX_AUTH_PRECEDENCE_WARNING);
            expect(CODEX_SANDBOX_AUTH_PRECEDENCE_WARNING_LOG_LINE).toMatch(/^\[paperclip\] Warning:/);
        });
        it("sandbox auth exists command is a static shell test", () => {
            expect(CODEX_SANDBOX_AUTH_EXISTS_COMMAND).toBe('test -f "$HOME/.codex/auth.json"');
...
//# sourceMappingURL=auth-precedence.test.js.map