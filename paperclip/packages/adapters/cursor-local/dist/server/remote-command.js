...
function prependPosixPathEntry(pathValue, entry) {
...
function prependPosixPathEntries(pathValue, entries) {
    return entries.reduceRight((value, entry) => prependPosixPathEntry(value, entry), pathValue);
}
function preferredSandboxCommandBasenames(command) {
    const basename = commandBasename(command);
    if (!DEFAULT_CURSOR_COMMAND_BASENAMES.has(basename))
        return [];
    return basename === "cursor-agent"
        ? ["cursor-agent", "agent"]
        : ["agent", "cursor-agent"];
}
function candidateSandboxCommandPaths(homeDir, basenames) {
    ...
    return CURSOR_SANDBOX_BIN_DIRS.flatMap((relativeDir) => basenames.map((basename) => path.posix.join(homeDir, relativeDir, basename)));
}
function candidateSandboxPathEntries(homeDir) {
    return CURSOR_SANDBOX_BIN_DIRS.map((relativeDir) => path.posix.join(homeDir, relativeDir));
}
function readMarkedValue(lines, marker) {
...
async function readSandboxCursorRuntimeInfo(input) {
    ...
    try {
        ...
        const fixedCandidatePaths = preferredBasenames.length > 0
            ? hintedRemoteSystemHomeDir
                ? candidateSandboxCommandPaths(hintedRemoteSystemHomeDir, preferredBasenames)
                : preferredBasenames.flatMap((basename) => CURSOR_SANDBOX_BIN_DIRS.map((relativeDir) => `$HOME/${relativeDir}/${basename}`))
            : [];
        const preferredProbeBranches = [
            ...fixedCandidatePaths.map((fixedPath) => `[ -x ${JSON.stringify(fixedPath)} ] && printf ${JSON.stringify(`${preferredMarker}%s\\n`)} ${JSON.stringify(fixedPath)}`),
            ...preferredBasenames.map((basename) => `resolved="$(command -v ${JSON.stringify(basename)} 2>/dev/null)" && [ -n "$resolved" ] && printf ${JSON.stringify(`${preferredMarker}%s\\n`)} "$resolved"`),
        ];
        const result = await runAdapterExecutionTargetShellCommand(input.runId, input.target, [
            hintedRemoteSystemHomeDir
...
export function isDefaultCursorCommand(command) {
...
export async function prepareCursorSandboxCommand(input) {
    ...
    const remoteSystemHomeDir = runtimeInfo.remoteSystemHomeDir ?? input.remoteSystemHomeDirHint?.trim() ?? null;
