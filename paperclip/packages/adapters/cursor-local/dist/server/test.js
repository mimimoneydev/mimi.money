import { asNumber, asString, asStringArray, parseObject, ensurePathInEnv, } from "@paperclipai/adapter-utils/server-utils";
...
export async function testEnvironment(ctx) {
    ...
    if (canRunProbe) {
        if (!isDefaultCursorCommand(command)) {
            ...
            const versionProbeTimeoutSec = Math.max(1, asNumber(config.versionProbeTimeoutSec, targetIsSandbox ? 60 : 45));
            ...
            const canRunHelloProbe = checks.every((check) => check.code !== "cursor_version_probe_failed" &&
