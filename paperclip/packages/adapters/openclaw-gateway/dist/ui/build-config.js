...
export function buildOpenClawGatewayConfig(v) {
    ...
    if (v.password)
        ac.password = v.password;
    if (v.agentId)
        ac.agentId = v.agentId;
    // Session routing fields
    if (v.sessionKeyStrategy)
        ac.sessionKeyStrategy = v.sessionKeyStrategy;
    if (v.sessionKey)
        ac.sessionKey = v.sessionKey;
    // Timeout fields
    if (typeof v.timeoutSec === "number")
        ac.timeoutSec = v.timeoutSec;
    if (typeof v.waitTimeoutMs === "number")
        ac.waitTimeoutMs = v.waitTimeoutMs;
    // Device auth fields
    if (typeof v.disableDeviceAuth === "boolean")
        ac.disableDeviceAuth = v.disableDeviceAuth;
    if (typeof v.autoPairOnFirstConnect === "boolean")
        ac.autoPairOnFirstConnect = v.autoPairOnFirstConnect;
    if (v.devicePrivateKeyPem)
        ac.devicePrivateKeyPem = v.devicePrivateKeyPem;
    ...
    if (v.paperclipApiUrl)
        ac.paperclipApiUrl = v.paperclipApiUrl;
    ...
    if (ac.timeoutSec == null)
        ac.timeoutSec = 120;
    if (ac.waitTimeoutMs == null)
        ac.waitTimeoutMs = 120000;
    if (!ac.sessionKeyStrategy)
