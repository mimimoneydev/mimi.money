...
function parseProviderConfig(raw, resolveEnv, notes) {
...
export async function prepareOpenCodeRuntimeConfig(input) {
    ...
    const gatewayProviders = parseProviderConfig(input.env.PAPERCLIP_OPENCODE_PROVIDERS ?? process.env.PAPERCLIP_OPENCODE_PROVIDERS, resolveEnv, notes);
    ...
    const nextProvider = gatewayProviders
        ...
        : existingProvider;
    if (gatewayProviders) {
        notes.push(`Injected ${Object.keys(gatewayProviders).length} custom OpenCode provider(s) from PAPERCLIP_OPENCODE_PROVIDERS: ${Object.keys(gatewayProviders).join(", ")}.`);
    }
    const nextConfig = {
        ...existingConfig,
        permission: {
