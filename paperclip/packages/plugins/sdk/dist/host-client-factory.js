...
export function createHostClientHandlers(options) {
    ...
    function resolveRequiredCompanyId(method, params, context) {
        if (context?.invalidInvocationScope) {
            throw new InvocationScopeDeniedError(pluginId, method, "the worker referenced a missing, expired, or unknown invocation scope");
        }
        const requested = requestedCompanyScope(method, params);
        ...
        if (requested.kind === "single") {
            ...
            if (requested.companyId !== scopedCompanyId) {
                throw new InvocationScopeDeniedError(pluginId, method, `requested company "${requested.companyId}" but the current invocation is scoped to company "${scopedCompanyId}"`);
            }
    ...
    return {
        ...
        "issues.createComment": gated("issues.createComment", async (params) => {
            if (params.actorUserId && !capabilitySet.has("issue.comments.create_human_attributed")) {
                throw new CapabilityDeniedError(pluginId, "issues.createComment", "issue.comments.create_human_attributed");
            }
