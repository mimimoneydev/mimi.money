...
import com.money.mimi.adapters.recyclerView.messages.MessagesAdapter;
import com.money.mimi.animations.AnimationsUtil;
import com.money.mimi.animations.ViewAudioProxy;
...
import com.money.mimi.helpers.Files.SafeFileGuard;
...
import com.money.mimi.presenters.messages.MessagesPresenter;
...
import com.money.mimi.ui.HideShowScrollListener;
            ...
            if (getIntent().hasExtra("messageCopied")) {
                ...
                for (String filepath : filePathList) {
                    if (FilesManager.getMimeType(filepath).equals("video/mp4")) {
                        FileVideoPath = FilesManager.persistFile(getApplicationContext(), filepath, FilesManager.FILE_TYPE_VIDEO);
                        MediaPlayer mp = MediaPlayer.create(this, Uri.fromFile(new File(FileVideoPath)));
                        if (mp != null) {
                            Duration = String.valueOf(mp.getDuration());
                            mp.release();
                        }
                        ...
                        FileVideoThumbnailPath = FilesManager.persistFile(getApplicationContext(), FilesManager.getPath(getApplicationContext(), FilesManager.getFile(fileVideo)), FilesManager.FILE_TYPE_THUMBNAIL);
                    } else if (FilesManager.getMimeType(filepath).equals("audio/mp3")) {
                        FileAudioPath = FilesManager.persistFile(getApplicationContext(), filepath, FilesManager.FILE_TYPE_AUDIO);
                        MediaPlayer mp = MediaPlayer.create(this, Uri.fromFile(new File(FileAudioPath)));
                        int duration = mp.getDuration();
                        Duration = String.valueOf(duration);
                        mp.release();
                    } else if (FilesManager.getMimeType(filepath).startsWith("application/") || FilesManager.getMimeType(filepath).startsWith("text/")) {
                        FileDocumentPath = FilesManager.persistFile(getApplicationContext(), filepath, FilesManager.FILE_TYPE_DOCUMENT);
                        File file = null;
                        ...
                        FileImagePath = FilesManager.persistFile(getApplicationContext(), filepath, FilesManager.FILE_TYPE_IMAGE);
                        File file = null;
...
*/
                        ...
                        if (!groupsModelMembers.get(x).isDeleted() && !groupsModelMembers.get(x).isLeft()) {
                            if (x <= 1) {
                                ...
                                if (groupsModelMembers.get(x).getUserId() == PreferenceManager.getID(this)) {
                                    ...
                                    finalName = getGroupMemberDisplayName(groupsModelMembers.get(x), 5);
    ...
    private void configureAttachmentButtons() {
        attachCamera.setImageResource(R.drawable.ic_photo_camera_white_48dp);
        attachImage.setImageResource(R.drawable.ic_photo_white_48dp);
        attachVideo.setImageResource(R.drawable.ic_attach_video_white_48dp);
        attachRecordVideo.setImageResource(R.drawable.ic_attach_videocam_white_48dp);
        attachDocument.setImageResource(R.drawable.ic_insert_drive_file_white_48dp);
        attachAudio.setImageResource(R.drawable.ic_audio_white_48dp);

        attachCamera.setContentDescription(getString(R.string.attach_camera));
        attachImage.setContentDescription(getString(R.string.attach_gallery));
        attachVideo.setContentDescription(getString(R.string.attach_video));
        attachRecordVideo.setContentDescription(getString(R.string.attach_video_record));
        attachDocument.setContentDescription(getString(R.string.this_a_pdf_file_document));
        attachAudio.setContentDescription(getString(R.string.attach_audio));
    }
    ...
    private void unBlockContact() {
        ...
        builderUnblock.setPositiveButton(R.string.Yes, (dialog, whichButton) -> {
            ...
            disposables.add(APIHelper.initialApiUsersContacts().unbBlock(recipientId).subscribe(blockResponse -> {
    ...
    private void blockContact() {
        ...
        builder2.setPositiveButton(R.string.Yes, (dialog, whichButton) -> {
            disposables.add(APIHelper.initialApiUsersContacts().block(recipientId).subscribe(blockResponse -> {
                if (blockResponse.isSuccess()) {
    ...
    private void addNewContact() {
        try {
            String walletAddress = mUsersModelRecipient != null ? mUsersModelRecipient.getWalletAddress() : "";
            ...
            dialog.setOnSaveWalletContactListener((firstName, lastName, address, category) -> {
                saveWalletContactToRealm(address, firstName, lastName, category);
                refreshMenu();
        ...
        try {
            ...
            realm.executeTransaction(r -> {
                ...
                if (existing != null) {
                    ...
                    existing.setLinked(true);
                    existing.setActivate(true);
                    AppHelper.LogCat("syncWalletContact: ensured wallet " + walletAddress + " exists in Realm before server sync");
                } else {
                    AppHelper.LogCat("syncWalletContact: wallet " + walletAddress + " NOT found in Realm before server sync!");
                }
            });
            if (!realm.isClosed()) realm.close();
        } catch (Exception e) {
        ...
        com.money.mimi.api.APIHelper.initialApiUsersContacts().updateContacts(contacts)
            .subscribe(contactsModelList -> {
                ...
                try {
                    ...
                    if (!realm2.isClosed()) realm2.close();
                } catch (Exception e) {
    ...
    private void launchImageChooser() {
        ...
        try {
            ...
            AppHelper.CustomToast(this, getString(R.string.no_application_to_show_image));
        }
    ...
    private void launchVideoChooser() {
        ...
        try {
            Intent intent = MediaPicker.createVideoIntent();
            startActivityForResult(intent, AppConstants.UPLOAD_VIDEO_REQUEST_CODE);
        } catch (ActivityNotFoundException ex) {
            AppHelper.CustomToast(this, getString(R.string.no_app_to_play_video));
        }
    ...
    private void launchDocumentChooser() {
        ...
        Intent intent = MediaPicker.createDocumentIntent("application/pdf");
        try {
    ...
    private void launchAudioChooser() {
        ...
        try {
            Intent intent = MediaPicker.createDocumentIntent("audio/*");
            startActivityForResult(
                    Intent.createChooser(intent, "Choose an audio"),
                    AppConstants.UPLOAD_AUDIO_REQUEST_CODE);
        } catch (ActivityNotFoundException ex) {
            AppHelper.CustomToast(this, getString(R.string.no_app_to_play_audio));
        }
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (resultCode == RESULT_OK) {
            if (data == null && requestCode != AppConstants.SELECT_MESSAGES_CAMERA
                    && requestCode != AppConstants.SELECT_MESSAGES_RECORD_VIDEO) {
                AppHelper.LogCat("Picker returned no data for request " + requestCode);
                return;
            }
                switch (requestCode) {
                    ...
                    case AppConstants.UPLOAD_PICTURE_REQUEST_CODE:
                        ...
                        if (FileImagePath != null) {
                            FileImagePath = FilesManager.persistFile(getApplicationContext(), FileImagePath, FilesManager.FILE_TYPE_IMAGE);
                            file = new File(FileImagePath);
                        }
                        if (file != null) {
                            FileSize = String.valueOf(file.length());
                    ...
                    case AppConstants.UPLOAD_VIDEO_REQUEST_CODE:
                        try {
                            ...
                            if (FileVideoPath != null) {
                                FileVideoPath = FilesManager.persistFile(getApplicationContext(), FileVideoPath, FilesManager.FILE_TYPE_VIDEO);
                            ...
                            try {
                                if (thumbnailBitmap != null) {
                                    ...
                                    fileVideo = null;
                                    AppHelper.LogCat("Video thumbnail is null, proceeding without thumbnail");
                                }
                            } catch (IOException e) {
                                AppHelper.LogCat("IOException video thumbnail " + e.getMessage());
                                fileVideo = null;
                            ...
                            if (fileVideo != null) {
                                if (AppHelper.isAndroid7()) {
                                    FileVideoThumbnailPath = FilesManager.persistFile(getApplicationContext(), fileVideo.getPath(), FilesManager.FILE_TYPE_THUMBNAIL);
                                } else {
                                    FileVideoThumbnailPath = FilesManager.persistFile(getApplicationContext(), FilesManager.getPath(getApplicationContext(), FilesManager.getFile(fileVideo)), FilesManager.FILE_TYPE_THUMBNAIL);
                                }
                            } else {
                    ...
                    case AppConstants.UPLOAD_AUDIO_REQUEST_CODE:
                        try {
                            ...
                            if (FileAudioPath != null) {
                                FileAudioPath = FilesManager.persistFile(getApplicationContext(), FileAudioPath, FilesManager.FILE_TYPE_AUDIO);
                            }
                            MediaPlayer mp = MediaPlayer.create(this, Uri.fromFile(new File(FileAudioPath)));
                            if (mp != null) {
                                Duration = String.valueOf(mp.getDuration());
                    ...
                    case AppConstants.UPLOAD_DOCUMENT_REQUEST_CODE:
                        ...
                        String documentMimeType = getContentResolver().getType(data.getData());
                        FileDocumentPath = FilesManager.getPath(getApplicationContext(), data.getData());
                        if (!SafeFileGuard.isSafeDocumentPath(FileDocumentPath, documentMimeType)) {
                            FileDocumentPath = null;
                            file = null;
                            AppHelper.CustomToast(MessagesActivity.this, SafeFileGuard.BLOCKED_FILE_MESSAGE);
                            break;
                        }
                        FileDocumentPath = FilesManager.persistFile(getApplicationContext(), FileDocumentPath, FilesManager.FILE_TYPE_DOCUMENT);
                        if (!SafeFileGuard.isSafeDocumentPath(FileDocumentPath, documentMimeType)) {
                            FileDocumentPath = null;
                            file = null;
                            AppHelper.CustomToast(MessagesActivity.this, SafeFileGuard.BLOCKED_FILE_MESSAGE);
    ...
    private void initializerMessageWrapper() {
        ...
        messageWrapper.addTextChangedListener(new TextWatcherAdapter() {
            ...
            public void onTextChanged(CharSequence s, int start, int before, int count) {
                ...
                if (isGroup) {
                    try {
                        if (mGroupsModel != null && mGroupsModel.getMembers() != null && mGroupsModel.getMembers().size() != 0) {
    ...
    private Runnable onTypingTimeout = new Runnable() {
        ...
        public void run() {
            ...
            isTyping = false;
            if (isGroup) {
                if (mGroupsModel == null || mGroupsModel.getMembers() == null) return;
    ...
    private void sendMessage() {
        ...
        senderId = PreferenceManager.getID(this);
        if (senderId == 0) {
            AppHelper.LogCat("Cannot send message: senderId is 0 (user not properly logged in)");
            return;
        }
        if (isGroup) {
            new Handler().postDelayed(() -> mMessagesPresenter.updateGroupConversationStatus(), 500);
        ...
        if (!FileSize.equals("0") && Long.parseLong(FileSize) > AppConstants.MAX_UPLOAD_FILE_SIZE) {
            AppHelper.CustomToast(this, getString(R.string.file_too_large, 50));
            FileImagePath = null;
            FileVideoPath = null;
            FileAudioPath = null;
            FileDocumentPath = null;
        ...
        if (isGroup) {
            ...
            try {
                ...
                try {
                    ...
                    messageGroup.put("walletAddress", mUsersModel.getWalletAddress());
            ...
            try {
                ...
                try {
                    ...
                    if (mUsersModelRecipient != null && mUsersModelRecipient.getWalletAddress() != null)
                        message.put("recipientWalletAddress", mUsersModelRecipient.getWalletAddress());
                } catch (Exception e) {
    ...
    private void unSentMessagesGroup(int groupID) {
        ...
        if (messagesModelsList.size() != 0 && mGroupsModel != null) {
    ...
    private void unSentMessagesForARecipient(int recipientID, boolean forFiles) {
        ...
        if (messagesModelsList.size() != 0) {
            for (MessagesModel messagesModel : messagesModelsList) {
                boolean hasFile = !messagesModel.getImageFile().equals("null")
                        || !messagesModel.getVideoFile().equals("null")
                        || !messagesModel.getAudioFile().equals("null")
                        || !messagesModel.getDocumentFile().equals("null");
                if (hasFile && !messagesModel.isFileUpload()) {
                    // File not uploaded yet — skip; upload will trigger EVENT_BUS_UPLOAD_MESSAGE_FILES when done
                    AppHelper.LogCat("unSentMessages: skipping message " + messagesModel.getId() + " — file not uploaded yet");
                } else if (hasFile && messagesModel.isFileUpload()) {
                    if (forFiles) {
    ...
    private void setStatusAsWaiting(JSONObject data, boolean isgroup) {
        ...
        try {
            if (isgroup) {
                ...
                String senderPhone = data.getString("walletAddress");
                ...
                realm.executeTransactionAsync(realm1 -> {
                    ...
                    UpdateMessageModel updateMessageModel = new UpdateMessageModel();
                        try {
                            ...
                            updateMessageModel.setWalletAddress(data.getString("walletAddress"));
                            ...
                            updateMessageModel.setUserToken(PreferenceManager.getToken(MessagesActivity.this));
                        } catch (JSONException e) {
                        e.printStackTrace();
                    }
                ...
                if (conversationID == 0) {
                    realm.executeTransactionAsync(realm1 -> {
                        ...
                        try {
                            ...
                            data.put("conversationId", lastConversationID);
                        } catch (JSONException e) {
                        ...
                        try {
                            ...
                            updateMessageModel.setSenderImage(data.getString("senderImage"));
                            updateMessageModel.setWalletAddress(data.getString("walletAddress"));
                            updateMessageModel.setRecipientWalletAddress(recipientPhone);
                            updateMessageModel.setDate(data.getString("date"));
                            updateMessageModel.setVideo(data.getString("video"));
                            updateMessageModel.setThumbnail(data.getString("thumbnail"));
                            updateMessageModel.setImage(data.getString("image"));
    ...
    private boolean checkIfContactExistsInRealm(String walletAddress) {
        try {
            Realm realm = WhatsCloneApplication.getRealmDatabaseInstance();
            RealmResults<ContactsModel> results = realm.where(ContactsModel.class)
                    .equalTo("walletAddress", walletAddress)
                    .findAll();
            ...
            AppHelper.LogCat("checkIfContactExistsInRealm error: " + e.getMessage());
            return false;
        }
    }

    private String getContactNameFromRealm(String walletAddress) {
    ...
    private String getGroupMemberDisplayName(MembersGroupModel member, int maxLength) {
        if (member == null) {
            return getString(R.string.app_name);
        }

        String walletAddress = member.getWalletAddress();
        String contactName = getContactNameFromRealm(walletAddress);
        if (contactName != null && !contactName.trim().isEmpty()) {
            return abbreviateName(contactName, maxLength);
        }

        if (walletAddress != null && !walletAddress.trim().isEmpty()) {
            return abbreviateName(walletAddress, maxLength);
        }

        String username = member.getUsername();
        if (username != null && !username.trim().isEmpty()) {
            return abbreviateName(username, maxLength);
        }

        return getString(R.string.app_name);
    }

    private String abbreviateName(String value, int maxLength) {
        if (value == null) {
            return getString(R.string.app_name);
        }

        String trimmedValue = value.trim();
        if (trimmedValue.isEmpty() || maxLength <= 0) {
            return getString(R.string.app_name);
        }

        return trimmedValue.substring(0, Math.min(trimmedValue.length(), maxLength));
    }
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        if (isGroup) {
            ...
            if (mUsersModelRecipient != null && mUsersModelRecipient.isValid())
                if (mUsersModelRecipient.getWalletAddress() != null && checkIfContactExistsInRealm(mUsersModelRecipient.getWalletAddress())) {
    ...
    private void emitMessageSeen() {
        if (isGroup) {
            ...
            try {
                ...
                Realm realm = WhatsCloneApplication.getRealmDatabaseInstance();
                try {
                    MessagesModel lastMsg = realm.where(MessagesModel.class)
                            .equalTo("senderID", recipientId)
                            .sort("id", Sort.DESCENDING)
                            .findFirst();
                    if (lastMsg != null) {
                        json.put("messageId", lastMsg.getId());
                    }
                } finally {
                    if (!realm.isClosed()) realm.close();
                }
    ...
    public void updateGroupInfo(GroupsModel groupsModel) {
        ...
        if (groupsModelMembers == null) {
            groupsModelMembers = new RealmList<>();
        }
        int arraySize = groupsModelMembers.size();
        StringBuilder names = new StringBuilder();
        for (int x = 0; x <= arraySize - 1; x++) {
    ...
    public void updateContactRecipient(ContactsModel contactsModels) {
        ...
        try {
            ...
            if (checkIfContactExistsInRealm(contactsModels.getWalletAddress())) {
            } else {
            }
            if (contactsModels.getUsername() != null) {
                ...
                name = getContactNameFromRealm(contactsModels.getWalletAddress());
                if (name != null) {
                    ToolbarTitle.setText(name);
                } else {
                    ToolbarTitle.setText(contactsModels.getWalletAddress());
                    name = contactsModels.getWalletAddress();
                }
            }
            ...
            for (int x = 0; x < arraySize; x++) {
                if (x <= 1) {
                    ...
                    if (groups.get(x).getUserId() == PreferenceManager.getID(this)) {
                        ...
                        finalName = getGroupMemberDisplayName(groups.get(x), 7);
                ...
                if (newUserNotification.getRecipientId() == recipientId) {
                    ...
                    if (newUserNotification.getAppName() != null && newUserNotification.getAppName().equals(getApplicationContext().getPackageName())) {
                        ...
                        if (newUserNotification.getFile() != null) {
                            NotificationsManager.showUserNotification(getApplicationContext(), newUserNotification.getConversationID(), newUserNotification.getWalletAddress(), newUserNotification.getFile(), recipientId, newUserNotification.getImage());
                        } else {
                            NotificationsManager.showUserNotification(getApplicationContext(), newUserNotification.getConversationID(), newUserNotification.getWalletAddress(), newUserNotification.getMessage(), recipientId, newUserNotification.getImage());
                        }
                    }
            ...
            case AppConstants.EVENT_BUS_NEW_GROUP_NOTIFICATION:
                ...
                if (newGroupNotification.getGroupID() == groupID) {
                    ...
                    if (newGroupNotification.getAppName() != null && newGroupNotification.getAppName().equals(getApplicationContext().getPackageName())) {
                        ...
                        String message;
                        String userName = getContactNameFromRealm(newGroupNotification.getWalletAddress());
                        switch (newGroupNotification.getMessage()) {
                            case AppConstants.CREATE_GROUP:
                                if (userName != null) {
                                    ...
                                    message = "" + newGroupNotification.getWalletAddress() + getApplicationContext().getString(R.string.he_created_this_group);
                                }
                            ...
                            case AppConstants.LEFT_GROUP:
                                if (userName != null) {
                                    ...
                                    message = "" + newGroupNotification.getWalletAddress() + getApplicationContext().getString(R.string.he_left);
                                }
            ...
            case AppConstants.EVENT_BUS_MEMBER_TYPING:
                if (pusher.getGroupID() != 0) {
                    if (!checkIfUserBlockedExist(pusher.getSenderID(), realm)) {
                        ...
                        String finalName;
                        String name = getContactNameFromRealm(contactsModel.getWalletAddress());
                        if (name != null) {
                            finalName = name;
                        } else {
                            finalName = contactsModel.getWalletAddress();
                        }
                            ...
                            if (messagesModel.getSenderID() == PreferenceManager.getID(this)) {
                                if (messagesModel.getVideoFile() != null && !messagesModel.getVideoFile().equals("null")) {
                                    ...
                                    if (FilesManager.isFileDocumentsSentExists(this, FilesManager.getDocument(messagesModel.getDocumentFile()))) {
                                        ...
                                        shareDocumentSafely(file, messagesModel.getMessage());
                                    } else {
                                        AppHelper.CustomToast(MessagesActivity.this, getString(R.string.this_document_is_not_exist));
                                    }
                                } else {
    ...
    private void shareDocumentSafely(File file, String message) {
        if (!SafeFileGuard.isSafeDocumentFile(file)) {
            AppHelper.CustomToast(MessagesActivity.this, SafeFileGuard.BLOCKED_FILE_MESSAGE);
            return;
        }
        AppHelper.shareIntent(file, this, message, AppConstants.SENT_DOCUMENTS);
    }
