...
public class MessagesPopupPresenter implements Presenter {
    ...
    public void onCreate() {
        ...
        disposables.add(mUsersContacts.getContactInfo(PreferenceManager.getID(view)).subscribe(view::updateContact, view::onErrorLoading));
        if (isGroup) {
            GroupsService mGroupsService = new GroupsService(realm, view.getApplicationContext(), mApiService);
            disposables.add(mGroupsService.getGroupInfo(GroupID).subscribe(view::updateGroupInfo, view::onErrorLoading));
            disposables.add(mGroupsService.updateGroupMembers(GroupID).subscribe(view::ShowGroupMembers, view::onErrorLoading));
            loadLocalGroupData();
    ...
    private void getRecipientInfo() {
        ...
        disposables.add(mUsersContacts.getContactInfo(RecipientID).subscribe(contactsModel -> {
    ...
    private void loadLocalGroupData() {
        ...
        disposables.add(mMessagesService.getConversation(ConversationID).subscribe(view::ShowMessages, view::onErrorLoading, view::onHideLoading));
    }
    ...
    private void loadLocalData() {
        ...
        try {
            disposables.add(mMessagesService.getConversation(ConversationID, RecipientID, PreferenceManager.getID(view)).subscribe(view::ShowMessages, view::onErrorLoading));
        } catch (Exception e) {
