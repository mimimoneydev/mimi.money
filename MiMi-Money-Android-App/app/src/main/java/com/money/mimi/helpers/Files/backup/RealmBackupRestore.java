...
public class RealmBackupRestore {
    ...
    public static int getMessageLastId() {
        Realm realm = WhatsCloneApplication.getRealmDatabaseInstance();
        try {
            Number maxId = realm.where(MessagesModel.class).max("id");
            if (maxId == null) {
                AppHelper.LogCat("messagesModelLastId null (empty table), returning 1");
                return 1;
            }
            AtomicInteger messagesModelLastId = new AtomicInteger(maxId.intValue());
    ...
    public static int getConversationLastId() {
        Realm realm = WhatsCloneApplication.getRealmDatabaseInstance();
        try {
            Number maxId = realm.where(ConversationsModel.class).max("id");
            if (maxId == null) {
                AppHelper.LogCat("conversationModelLastId null (empty table), returning 1");
                return 1;
            }
            AtomicInteger conversationModelLastId = new AtomicInteger(maxId.intValue());
    ...
    public static int getCallLastId() {
        Realm realm = WhatsCloneApplication.getRealmDatabaseInstance();
        try {
            Number maxId = realm.where(CallsModel.class).max("id");
            if (maxId == null) {
                AppHelper.LogCat("callsModelLastId null (empty table), returning 1");
                return 1;
            }
            AtomicInteger callsModelLastId = new AtomicInteger(maxId.intValue());
    ...
    public static int getCallInfoLastId() {
        Realm realm = WhatsCloneApplication.getRealmDatabaseInstance();
        try {
            Number maxId = realm.where(CallsInfoModel.class).max("id");
            if (maxId == null) {
                AppHelper.LogCat("CallsInfoModelLastId null (empty table), returning 1");
                return 1;
            }
            AtomicInteger CallsInfoModelLastId = new AtomicInteger(maxId.intValue());
    ...
    public static int getBlockUserLastId() {
        Realm realm = WhatsCloneApplication.getRealmDatabaseInstance();
        try {
            Number maxId = realm.where(UsersBlockModel.class).max("id");
            if (maxId == null) {
                AppHelper.LogCat("BlockModelLastId null (empty table), returning 1");
                return 1;
            }
            AtomicInteger BlockModelLastId = new AtomicInteger(maxId.intValue());
